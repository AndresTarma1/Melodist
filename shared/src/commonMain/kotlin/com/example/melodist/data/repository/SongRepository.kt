...
class SongRepositoryImpl(private val database: MelodistDatabase) : SongRepository {
    override fun getSavedSongs(): Flow<List<SongItem>> {
        return database.savedSongQueries.selectAll()
            .asFlow()
            .mapToList(Dispatchers.IO)
            .map { list -> list.map { savedSongToSongItem(it) } }
    }

    override fun isSongSaved(id: String): Flow<Boolean> {
    ...
    override suspend fun saveSong(song: SongItem) = withContext(Dispatchers.IO) {
    ...
    override suspend fun removeSong(id: String) = withContext(Dispatchers.IO) {
        database.savedSongQueries.delete(id)
    }

    override suspend fun getDownloadedSongs(): List<SongItem> = withContext(Dispatchers.IO) {
